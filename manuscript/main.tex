...
\begin{figure}[h!]
  ...
  \caption{Overview of \method{}. (1) A risk tagger marks each report token as ordinary or clinically
  critical (negation, laterality, severity, measurement, finding). (2) Tokens are routed through the
  layers by risk: ordinary tokens take a cheap confidence exit at a shallow layer, while critical
  tokens are held until they reach a minimum depth and the same top prediction has been stable for $k$
  consecutive layers. (3) The result keeps most of the compute saving on ordinary tokens while
  preserving the full-model decision on the tokens whose errors would change the clinical meaning.}
  \label{fig:overview}
\end{figure}
...
on the tokens that determine the meaning of the report. \method{} shows that a small, training-free change to
the halting rule, spending a few extra layers only on high-risk tokens, removes most of that
concentrated error.

\paragraph{Where this is useful, concretely.}
We do not claim that adaptive depth should replace quantization, since the two compose. The most
realistic deployment is self-speculative decoding, where early-exit drafts are accepted if they match
the full model and otherwise corrected. Standard acceptance is lossless but spends full-model compute on
every token. A harm-aware variant can accept cheap drafts for ordinary tokens while requiring full-model
confirmation only for critical ones, which recovers latency without the clinical risk that uniform
aggressive acceptance would incur. The risk tagger and stability criterion of \method{} are exactly the
acceptance policy that such a system needs.

\section{Threats to Validity}
\label{sec:threats}
\textbf{Construct.} Preservation and excess gold-token error are token-level proxies, and the CheXbert
counterfactual measures finding changes on perturbed gold text, not radiologist-judged report
correctness. \textbf{Internal.} The logit lens is a known-noisy probe, and the architectures expose a
final norm that we reuse. Results are reported relative to the same probe for all policies, so the
comparison is fair even if the absolute lens is imperfect. \textbf{External.} The domain is chest
radiology and the language is English, the risk tagger is a lexicon (though pluggable), and one model
(Phi-3.5-mini) was excluded for a remote-code cache incompatibility, leaving two families.
\textbf{Statistical.} Intervals are report-level bootstraps. The intervals for the main effect do not overlap,
but the smaller runs are illustrative rather than definitive.
